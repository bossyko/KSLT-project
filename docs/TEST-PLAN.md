...
│   ├── challenges.spec.js    — Challenge Battle (voting, cards, admin)
│   ├── loyalty.spec.js       — Loyalty Program (points, rewards)
...
## TC-12: Challenge Battle

| # | Тест-кейс | Шаги | Ожидание |
|---|-----------|------|----------|
| 12.1 | Battle cards на главной (гость) | Без авторизации → Главная | Карточки видны в 3 колонки, "Войдите чтобы голосовать" |
| 12.2 | Battle cards на главной (авторизованный) | С авторизацией → Главная | Карточки с кнопками голосования |
| 12.3 | Battle cards на турнирах | Tournaments overview → секция "Баттлы" | Карточки с VS, дата, голосование |
| 12.4 | Battle cards: responsive | Viewport 480px | 1 колонка; 768px → 2 колонки |
| 12.5 | Battle card: клик "Подробнее" | Клик на ссылку карточки | Переход на challenge.html?id=X |
| 12.6 | Challenge page: VS layout | GET /pages/challenge.html?id=X | Фото игроков, имена, статистика, VS |
| 12.7 | Challenge page: гость | Без авторизации → challenge page | VS layout видим, "Войдите чтобы голосовать" |
| 12.8 | Challenge page: breadcrumb | Перейти с главной → challenge | Sticky breadcrumb "← Главная / Баттлы" |
| 12.9 | Challenge page: breadcrumb из турниров | Перейти с tournaments-overview → challenge | Sticky breadcrumb "← Турниры / Баттлы" |
| 12.10 | Голосование: авторизованный | Авторизация → challenge page | Кнопки голосования активны |
| 12.11 | Голосование: проголосовать | Клик на кнопку → RPC cast_battle_vote | Toast "Голос принят!", % обновляется |
| 12.12 | Голосование: повторно | Попытка проголосовать ещё раз | "Ваш голос" (кнопки disabled) |
| 12.13 | Голосование: авто-закрытие | Текущее время >= дата/время матча | "Голосование закрыто" + кнопки disabled |
| 12.14 | Голосование: "до" показ | Голосование ещё открыто | "Голосование открыто до 25.03 18:00" |
| 12.15 | H2H: история встреч | Challenge page с матчами | Таблица предыдущих встреч |
| 12.16 | Score: результат | Матч завершён | Счёт по сетам + "Победитель: ..." + % угадали |
| 12.17 | TG: inline кнопки | Анонс в TG группе | 2 кнопки (🔴 Игрок1 / 🔵 Игрок2) + "Подробнее" |
| 12.18 | TG: голосование | Нажать inline кнопку в TG | "Голос за ... принят!", кнопки обновляются с % |
| 12.19 | TG: повторное голосование | Нажать кнопку после голосования | "Вы уже проголосовали!" |
| 12.20 | Кросс-проверка: TG→сайт | Проголосовать в TG → попытка на сайте | "Вы уже голосовали в Telegram!" |
| 12.21 | Кросс-проверка: сайт→TG | Проголосовать на сайте → попытка в TG | "Вы уже голосовали на сайте!" |
| 12.22 | Admin: опубликовать баттл | Админка → Challenges → Опубликовать | Модалка с title + banner, анонс в TG |
| 12.23 | Admin: broadcast | Админка → Challenges → 📢 Отправить | TG отправлен, статус "Отправлено" |
| 12.24 | Admin: ввести счёт | Админка → Challenges → Ввести счёт | Модалка сетов, auto-winner, сохранение |
| 12.25 | Admin: модалка закрытие | Сохранить счёт → модалка | Модалка закрывается при успехе |
| 12.26 | Admin: удалить баттл | Админка → Challenges → 🗑 | Подтверждение → удаление из БД |
| 12.27 | Admin: кликабельный title | Клик на название баттла | Открывается challenge.html в новой вкладке |
| 12.28 | 3 языка: challenge page | RU/EN/KG версии challenge.html | Labels на каждом языке корректны |

## TC-13: Loyalty Program

| # | Тест-кейс | Шаги | Ожидание |
|---|-----------|------|----------|
| 13.1 | Dashboard: вкладка Баллы | Dashboard → Баллы | Баланс, история, список наград |
| 13.2 | Auto-earn: оплата | Создать payment (court/coach/membership) | Начислены баллы по правилу |
| 13.3 | Welcome bonus | Первое членство → 200 баллов | Разовое начисление |
| 13.4 | Redeem: обмен на награду | Обменять баллы → награда | Баланс уменьшился, транзакция записана |
| 13.5 | Redeem: недостаточно баллов | Попытка обмена без баланса | Ошибка "Недостаточно баллов" |
| 13.6 | Admin: правила | Админка → Лояльность → Правила | CRUD правил начисления |
| 13.7 | Admin: награды | Админка → Лояльность → Награды | CRUD наград для обмена |
| 13.8 | Admin: транзакции | Админка → Лояльность → Транзакции | Таблица всех начислений/списаний |
| 13.9 | Admin: ручная корректировка | Админка → adjust баллы | Начислить/списать вручную |
| 13.10 | Expiry: 12 мес | Баллы старше 12 мес | pg_cron: баллы сгорают |
...
- TC-08 (PWA)
- TC-12.8-12.12 (голосование: core flow + защита)
...
- TC-09 (адаптивность)
- TC-10 (мультиязычность)
- TC-12.1-12.7 (battle cards + challenge page)
- TC-13 (loyalty program)
...
- TC-07.4+ (админ CRUD)
- TC-11, TC-14
- TC-12.16-12.27 (TG voting + admin battles)
