...
| # | Тест-кейс | Шаги | Ожидание |
|---|-----------|------|----------|
| 14.1 | Info overview | GET /pages/info.html | Ссылки на about, rules, faq, pricing, offer |
| 14.2 | About page | GET /pages/about.html | Контент "О нас" |
| 14.3 | FAQ page | GET /pages/faq.html | Accordion вопросы/ответы |
| 14.4 | Pricing page | GET /pages/pricing.html | Тарифы + сравнение |
| 14.5 | Rules page | GET /pages/rules.html | Правила KSLT |

## TC-15: Завершение матча (Duel / Challenge)

| # | Тест-кейс | Шаги | Ожидание |
|---|-----------|------|----------|
| 15.1 | Модалка: открытие | Админка → Вызовы → Опубликованные → "Ввести счёт" | Модалка с полями сетов |
| 15.2 | Сохранить черновик | Ввести 1 сет (6-4) → "Сохранить" | Toast "Счёт сохранён", модалка остаётся открытой |
| 15.3 | Черновик: pre-fill | Закрыть модалку → снова открыть | Поля заполнены из score_draft |
| 15.4 | Завершить: 2-0 | Ввести 6-4, 7-5 → "Завершить матч" | Победитель определён, модалка закрылась |
| 15.5 | Завершить: 2-1 | Ввести 6-4, 3-6, 7-6 → "Завершить матч" | Победитель по 2 из 3 сетов |
| 15.6 | Валидация: нет победителя | Ввести 6-4, 4-6 (1-1) → "Завершить" | Ошибка "Определите победителя" |
| 15.7 | Match type = duel | Проверить запись в matches | match_type = 'duel', status = 'completed' |
| 15.8 | Challenge status | Проверить challenge после завершения | status = 'completed', match_id заполнен, score_draft = null |
| 15.9 | Wins +1 | Проверить players.wins у победителя | Инкремент на 1 |
| 15.10 | Losses +1 | Проверить players.losses у проигравшего | Инкремент на 1 |
| 15.11 | Form обновлена | Проверить players.form обоих | Последние 5 матчей (W/L массив) |
| 15.12 | Voting closed | Проверить challenge.voting_closed | true после завершения |
| 15.13 | Вкладка "Завершённые" | Админка → Вызовы → Завершённые | Завершённый баттл в таблице |
| 15.14 | Дуэль не в матчах | Dashboard → Мои Игры → Матчи | Дуэль отфильтрована, только турнирные |
| 15.15 | Дуэль в вызовах | Dashboard → Мои Игры → Вызовы | Дуэль отображается |
| 15.16 | H2H метка | Публичная страница challenge.html → H2H | "(Вызов)" / "(Duel)" рядом с матчем |
| 15.17 | recalcPlayerPoints | Финализировать турнир | Duel wins/losses не затёрты |

## TC-16: Дашборд админки — быстрые действия

| # | Тест-кейс | Шаги | Ожидание |
|---|-----------|------|----------|
| 16.1 | Таблица: колонки | Дашборд → Ожидают одобрения | Игрок, Категория, Рейтинг, Турнир, Дата, Действия |
| 16.2 | Кнопка ✓ | Клик зелёную ✓ | Status → 'approved', toast, таблица обновлена |
| 16.3 | Кнопка ✗ | Клик красную ✗ | Status → 'rejected', toast, таблица обновлена |
| 16.4 | Кнопка disabled | Клик ✓ → повторный клик | Кнопка disabled, нет дублирования |
| 16.5 | Клик турнир | Клик на название турнира | Переход на сетку турнира, без мигания |
| 16.6 | Клик игрок | Клик на имя игрока | Переход на карточку игрока в админке |
| 16.7 | Категория/рейтинг | Проверить данные | Корректная категория и очки из players |
| 16.8 | Count badge | После одобрения/отклонения | Счётчик уменьшился |

## TC-17: Отклонённые заявки (bracket)

| # | Тест-кейс | Шаги | Ожидание |
|---|-----------|------|----------|
| 17.1 | Admin видит | Admin → Турнир → Заявки | Секция "Отклонённые" после Листа ожидания |
| 17.2 | Manager не видит | Manager → Турнир → Заявки | Секции "Отклонённые" нет |
| 17.3 | Красный заголовок | Есть rejected заявки | Заголовок красным, badge с количеством |
| 17.4 | Нет rejected | Нет отклонённых заявок | Секция не отображается |
| 17.5 | Полупрозрачность | Отклонённые строки | opacity: 0.6 |
| 17.6 | Колонки | Строка rejected | №, Ранг, ФИО, Категория, Дата регистрации |
| 17.7 | Без чекбоксов | Строки rejected | Нет чекбоксов (только просмотр) |

## TC-18: Профиль игрока — порядок секций

| # | Тест-кейс | Шаги | Ожидание |
|---|-----------|------|----------|
| 18.1 | Порядок (dashboard) | Dashboard → Мои Игры | Матчи → Вызовы → Турниры, Достижения внизу |
| 18.2 | Порядок (player.html) | Публичная страница игрока | Матчи → Вызовы → Турниры, Достижения внизу |
| 18.3 | Матчи раскрыты | Секция "Матчи" | Раскрыта по умолчанию |
| 18.4 | Вызовы свёрнуты | Секция "Вызовы" | Свёрнута по умолчанию |
| 18.5 | Турниры раскрыты | Секция "Турниры" | Раскрыта по умолчанию |
| 18.6 | Фильтр duel | Секция "Матчи" | Только tournament, без duel |
| 18.7 | Достижения внизу | Скролл вниз | Секция после "Мои Игры" |
...
- TC-15.1-15.7 (завершение матча: core flow)
- TC-16.2-16.3 (быстрые действия дашборда)
...
- TC-13 (loyalty program)
- TC-15.8-15.17 (duel stats, фильтры, recalc)
- TC-17 (отклонённые заявки — access control)
- TC-18 (порядок секций профиля)
...
- TC-16.4-16.8 (дашборд: навигация, disable)
