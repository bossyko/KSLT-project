...
(function() {
  ...
  B.load = function() {
    ...
    supabaseClient.from('challenges')
      ...
      .catch(function() {
        ...
        el.innerHTML = '<div class="empty-state"><div class="empty-icon">⚔️</div><div class="empty-title">Баттлы</div><div class="empty-text">Скоро здесь появятся баттлы</div></div>';
      });
  };
  ...
  function render() {
    ...
    if (filtered.length === 0) {
      el.innerHTML = '<div class="empty-state"><div class="empty-icon">⚔️</div><div class="empty-title">Нет баттлов</div><div class="empty-text">' +
        (currentFilter === 'active' ? 'Активных баттлов пока нет' : 'Завершённых баттлов пока нет') + '</div></div>';
      return;
    }

    var html = '';
    filtered.forEach(function(b) {
      var name1 = (b.challenger_player && b.challenger_player.name) || (b.challenger && b.challenger.full_name) || 'Игрок 1';
      var name2 = (b.opponent_player && b.opponent_player.name) || 'Игрок 2';

      html += '<div class="battle-card" data-battle-id="' + b.id + '">' +
        '<div class="battle-title">' + esc(b.battle_title || 'Баттл') + '</div>' +
        '<div class="battle-vs">' +
        ...
        '<div class="battle-btn-row">' +
          '<button class="battle-btn primary">Подробнее</button>' +
        '</div>' +
      '</div>';
    });
    el.innerHTML = html;
