...
(function() {
  ...
  PD.open = function(playerId) {
    ...
    supabaseClient.from('players')
      ...
      .then(function(r) {
        if (r.error || !r.data) {
          detail.innerHTML = '<div class="pd-empty"><div class="pd-empty-icon">🔍</div><div class="pd-empty-title">Игрок не найден</div></div>';
          return;
        }
        _player = r.data;
  ...
  function renderGuestView() {
    ...
    html += '<h3>Зарегистрируйтесь</h3>';
    html += '<p>Полная статистика, матчи, достижения и вызовы доступны после входа</p>';
    html += '<button class="pd-guest-btn" id="pdLoginBtn">Войти / Регистрация</button>';
    html += '</div></div>';
  ...
  function renderFullProfile(access) {
    ...
    html += pdStatCard(p.points || 0, 'Рейтинг', p.rank_change);
    html += pdStatCard(p.wins || 0, 'Победы', null);
    html += pdStatCard(p.losses || 0, 'Поражения', null);
    html += pdStatCard(winRate + '%', '% Побед', null);
    // Streak with form dots
    html += '<div class="pd-stat">';
    html += '<div class="pd-stat-num">' + streak.count + (streak.type === 'up' ? 'W' : 'L') + '</div>';
    html += '<div class="pd-stat-label">Серия</div>';
    ...
    if (access === 'member') {
      ...
      html += '<button class="pd-challenge-btn" id="pdChallengeBtn">⚔️ Вызвать на матч</button>';
      html += '</div>';
    } else if (access === 'registered') {
      html += '<div class="pd-cta-membership">';
      html += '<p>Оформите членство KSLT, чтобы бросить вызов игрокам</p>';
      html += '</div>';
    }
    ...
    supabaseClient.from('matches')
      ...
      .then(function(r) {
        if (r.error || !r.data || r.data.length === 0) {
          container.innerHTML = '<div class="pd-empty-small">Нет данных о матчах</div>';
          return;
        }
  ...
  function renderMatches(container, matches) {
    ...
    visible.forEach(function(m) {
      ...
      html += '<div class="pd-match-opp" id="pdOpp_' + esc(oppId) + '">Загрузка...</div>';
      html += '</div>';
    ...
    if (!showAll) {
      html += '<button class="pd-show-all" id="pdShowAllMatches">Показать все (' + matches.length + ')</button>';
    }
  ...
  function loadTournaments() {
    ...
    supabaseClient.from('tournament_registrations')
      ...
      .then(function(r) {
        if (r.error || !r.data || r.data.length === 0) {
          container.innerHTML = '<div class="pd-empty-small">Нет данных о турнирах</div>';
          return;
        }
        ...
        r.data.forEach(function(reg) {
          ...
          html += '<div class="pd-tournament-status ' + statusClass + '">' + (t.status === 'completed' ? 'Завершён' : 'Предстоит') + '</div>';
          html += '</div>';
        });
        html += '</div>';
        container.innerHTML = html;
  ...
  function renderAchievements() {
    ...
    if (_allBadges.length === 0 && _earnedBadges.length === 0) {
      container.innerHTML = '<div class="pd-empty-small">Нет бейджей</div>';
      return;
    }
  ...
  function loadH2HData(opponentId) {
    ...
    Promise.all([oppPromise, matchesPromise]).then(function(results) {
      ...
      if (!opp) {
        body.innerHTML = '<div class="pd-empty-small">Данные не найдены</div>';
        return;
      }
      ...
      html += '<div class="pd-h2h-stat"><span class="pd-h2h-val my">' + myWins + '</span><span class="pd-h2h-label">Победы</span><span class="pd-h2h-val opp">' + oppWins + '</span></div>';
      html += '<div class="pd-h2h-stat"><span class="pd-h2h-val my">' + mySets + '</span><span class="pd-h2h-label">Сеты</span><span class="pd-h2h-val opp">' + oppSets + '</span></div>';
      html += '<div class="pd-h2h-stat"><span class="pd-h2h-val my">' + myGames + '</span><span class="pd-h2h-label">Геймы</span><span class="pd-h2h-val opp">' + oppGames + '</span></div>';
      html += '</div>';

      // Match list
      if (matches.length === 0) {
        html += '<div class="pd-empty-small">Матчей между игроками не найдено</div>';
      } else {
        html += '<h4 class="pd-h2h-matches-title">Последние матчи</h4>';
  ...
  function handleChallenge() {
    ...
    if (!AUTH || !AUTH.currentUser) {
      window.KSLT_APP.toast('Войдите в аккаунт');
      return;
    }
    if (!AUTH.currentProfile || !AUTH.currentProfile.player_id) {
      window.KSLT_APP.toast('Привяжите профиль игрока');
      return;
    }
    if (AUTH.currentProfile.player_id === _playerId) {
      window.KSLT_APP.toast('Нельзя вызвать самого себя');
      return;
    }
    ...
    supabaseClient.from('challenges')
      ...
      .then(function(r) {
        if (r.data && r.data.length > 0) {
          window.KSLT_APP.toast('У вас уже есть активный вызов этому игроку');
          return;
        }
        // Create challenge
        supabaseClient.from('challenges').insert({
          ...
          if (res.error) {
            window.KSLT_APP.toast('Ошибка: ' + (res.error.message || 'Попробуйте позже'));
          } else {
            window.KSLT_APP.toast('Вызов отправлен!');
          }
        });
      });
  }
