...
(function() {
  ...
  function render() {
    ...
    if (courtsToShow.length === 0) {
      el.innerHTML = '<div class="empty-state"><div class="empty-icon">🏟</div><div class="empty-title">Нет кортов</div></div>';
      return;
    }
    ...
    for (var i = 0; i < visibleCount; i++) {
      ...
      html += '<span>🎾 ' + c.totalCourts + ' ' + courtWord(c.totalCourts) + '</span>';
      ...
      if (c.minPrice > 0) html += '<div class="court-card-price">от ' + c.minPrice + ' сом/час</div>';
      html += '</div></div>';
    }
  ...
  function openCourtDetail(c, isMember) {
    ...
    html += '<span>🎾 ' + c.totalCourts + ' кортов</span>';
    html += '<span>' + typeLabel + '</span>';
    if (c.surface) html += '<span>' + esc(c.surface) + '</span>';
    html += '</div>';
