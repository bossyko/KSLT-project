...
(function() {
    ...
    function renderBattleCard(battle, players, votes) {
        ...
        // Date/time as dark pills for universal readability
        var headerMeta = '';
        if (date || time) {
            headerMeta = '<div class="bc-header-meta">' +
                (date ? '<span class="bc-date-pill">\uD83D\uDCC5 ' + formatDate(date) + '</span>' : '') +
                (time ? '<span class="bc-date-pill">\u23F0 ' + esc(time) + '</span>' : '') +
            '</div>';
        }
    ...
    function inject(containerId, data) {
        ...
        container.innerHTML =
            '<div class="bc-section-header"><h2>\u2694\uFE0F ' + L.battle + '</h2></div>' +
