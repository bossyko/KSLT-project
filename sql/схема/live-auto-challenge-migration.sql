...
-- 4. Update get_player_challenges — fallback to live_matches for names
CREATE OR REPLACE FUNCTION get_player_challenges(p_player_id TEXT)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE result JSON;
BEGIN
    SELECT json_agg(row_to_json(t)) INTO result FROM (
        SELECT c.id, c.status, c.proposed_date, c.proposed_time, c.proposed_venue, c.counter_date, c.counter_time, c.counter_venue, c.created_at, c.accepted_at,
            c.challenger_player_id, COALESCE(cp.name, lm.player1_name) AS challenger_name, COALESCE(cp.name_en, lm.player1_name) AS challenger_name_en, COALESCE(cp.name_kg, lm.player1_name) AS challenger_name_kg, cp.photo AS challenger_photo,
            c.opponent_player_id, COALESCE(op.name, lm.player2_name) AS opponent_name, COALESCE(op.name_en, lm.player2_name) AS opponent_name_en, COALESCE(op.name_kg, lm.player2_name) AS opponent_name_kg, op.photo AS opponent_photo,
            ct.name AS court_name, cct.name AS counter_court_name, COALESCE(m.score, c.score_draft) AS match_score, m.winner_id AS match_winner_id
        FROM challenges c
        ...
        WHERE (c.challenger_player_id = p_player_id OR c.opponent_player_id = p_player_id) AND c.status IN ('accepted', 'completed')
...
BEGIN
    SELECT json_agg(row_to_json(t)) INTO result FROM (
        SELECT c.id, c.status, c.proposed_date, c.proposed_time, c.proposed_venue, c.counter_date, c.counter_time, c.counter_venue, c.message, c.created_at, c.expires_at, c.accepted_at, c.countered_at, c.match_id,
            CASE WHEN c.challenger_id = auth.uid() THEN 'sent' ELSE 'received' END AS direction,
            c.challenger_player_id, cp.full_name AS challenger_name, cp.avatar_url AS challenger_avatar,
            c.opponent_player_id, op.full_name AS opponent_name, op.avatar_url AS opponent_avatar,
            ct.name AS court_name, cct.name AS counter_court_name, COALESCE(m.score, c.score_draft) AS match_score, m.winner_id AS match_winner_id
        FROM challenges c
        ...
        LEFT JOIN courts cct ON cct.id = c.counter_court_id
        LEFT JOIN matches m ON m.id = c.match_id
        WHERE c.challenger_id = auth.uid() OR c.opponent_profile_id = auth.uid()
...
CREATE POLICY challenges_completed_read ON challenges FOR SELECT TO authenticated USING (status = 'completed');
