-- ═════════════════════════════════════════════════════════════════════════
-- НЕ ЗАПУСКАТЬ. УВЕДЁН В АРХИВ 24.09.2026.
--
-- Этот файл (март) завёл четыре значка под id champion_x3, top_1, top_10 и
-- dominator. Позже badges-44.sql (август, полный посев на 44 значка) завёл
-- ТЕ ЖЕ четыре достижения под id champion_3, rank_1, rank_10 и domination,
-- а старые строки никто не удалил.
--
-- Выдача сопоставляет значки по УСЛОВИЮ, а не по id, поэтому срабатывали оба
-- определения, и дубль размножался при каждом пересчёте: за час 6/6 стало
-- 7/7, а 50/50 — 51/51.
--
-- Дубли удалены запросом sql/обслуживание/dubli-znachkov.sql. Файл уведён
-- сюда, потому что иначе его следующий прогон — хоть на чистом окружении —
-- создаст их заново. Живой посев значков живёт в badges-44.sql.
-- ═════════════════════════════════════════════════════════════════════════
