    ...
    --
    -- Историю замен сносим вместе с остальным: она описывает состав, которого
-- после сброса не существует.
--
-- Лист ожидания не трогаем: это честная очередь подачи, а не след проверок.
    ...
    -- История замен: после сброса она говорит неправду. Журнал описывает
    -- состав, которого больше нет: «такого-то заменили на такого-то», хотя
    -- эти люди либо не в турнире, либо вернулись на свои места
