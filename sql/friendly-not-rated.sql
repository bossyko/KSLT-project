-- ============================================
-- Friendly не должен попадать в очки по категориям
-- ============================================
--
-- Friendly — турниры для практики: очки за них не начисляются никогда, и в
-- рейтинге эта категория не участвует. Так устроено при разборе сеток
-- (js/admin/sections/bracket.js) и на странице рейтинга.
--
-- Но запись в историю можно добавить руками из админки, и там Friendly был
-- в списке категорий. Одна такая запись — и у игрока появлялась строка
-- «Friendly · 50 очков» на публичной странице и в кабинете.
--
-- Убираем Friendly из расчёта и подчищаем то, что уже насчиталось.
--
...
-- ---- 3. Подчистить то, что уже насчиталось ----------------------------
DELETE FROM player_categories WHERE category_id = 'friendly';


-- ---- 4. ПРОВЕРКА ------------------------------------------------------
-- Должно быть 0. Записи в истории про Friendly остаются: игры сыграны,
-- просто очков за них нет.
SELECT
    (SELECT count(*) FROM player_categories WHERE category_id = 'friendly') AS осталось_строк_friendly,
    (SELECT count(*) FROM rating_history WHERE category_id = 'friendly') AS записей_в_истории;
