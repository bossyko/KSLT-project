...
// Новый проект — не беда, а новость: сравнивать не с чем, потому что
// такой ширины раньше не мерили. Отказываться из-за него от всего
// сравнения было бы перебором, а молча подставлять «до» — враньём.
const новые = [];
for (const k of после.keys()) if (!до.has(k)) новые.push(k);
...
const строки = [];
if (новые.length) {
    const проекты = [...new Set(новые.map(f => f.split('__')[0]))];
    console.log('\n  НОВОЕ, сравнивать не с чем: ' + проекты.join(', ') +
        '  (' + новые.length + ' снимков)');
}
