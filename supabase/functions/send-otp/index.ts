  ...
  async function загрузитьТексты(supabase: any) {
    if (_тексты) return _тексты
    try {
      const { data } = await supabase.from('notification_texts').select('key, ru, kg, en')
      _тексты = {}
      for (const строка of (data || [])) {
        _тексты[строка.key] = { ru: строка.ru, kg: строка.kg, en: строка.en }
      }
    } catch {
      _тексты = {}
    }
  ...
  async function sendTelegramOtp(chatId: string, code: string, flow: string, язык: Язык = 'ru') {
    const token = Deno.env.get('TELEGRAM_BOT_TOKEN')!
    const flowLabel = flow === 'forgot_password'
      ? т('mail_otp_flow_forgot', язык)
      : т('mail_otp_flow_register', язык)

    const text =
      `🔐 *${т('mail_otp_subject', язык)}*\n\n` +
      т('mail_otp_body', язык, { 'действие': flowLabel, 'код': '`' + code + '`' })
  ...
  async function sendEmailOtp(email: string, code: string, flow: string, язык: Язык = 'ru') {
    ...
    const res = await fetch(`${supabaseUrl}/functions/v1/send-email`, {
      ...
      body: JSON.stringify({
        to: email,
        subject: т('mail_otp_subject', язык),
        template: 'otp-code',
        data: { code, flow, lang: язык },
      }),
    })
  ...
  Deno.serve(async (req) => {
    ...
    try {
      ...
      // При регистрации профиля ещё нет — язык присылает страница, с которой
      // человек регистрируется. При восстановлении берём из профиля
      let языкПолучателя: Язык = (body.lang === 'en' || body.lang === 'kg') ? body.lang : 'ru'
      ...
      if (flow === 'telegram_register' && telegramChatId) {
        ...
        const query = db.from('profiles').select('id, telegram_chat_id, lang')
        ...
        // Язык человека: код придёт на том, на котором он читает сайт
        языкПолучателя = (profile.lang === 'en' || profile.lang === 'kg') ? profile.lang : 'ru'
      ...
      if (channel === 'telegram' && deliveryChatId) {
        await sendTelegramOtp(deliveryChatId, code, flow, языкПолучателя)
      } else if (deliveryEmail) {
        await sendEmailOtp(deliveryEmail, code, flow, языкПолучателя)
      }
