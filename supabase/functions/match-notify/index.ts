  ...
  Deno.serve(async (req) => {
    ...
    try {
      ...
      let body: {
        tournament_id?: string; match_id?: string; kind?: string;
        entry?: { registration_id: string; event: string };
        replacement?: { tournament_id: string; registration_id: string;
                        old_player_id?: string | null; new_player_id?: string | null;
      ...
      if (body.entry) {
        // ---- Решение менеджера по заявке: игроку и напарнику ----
        return await notifyEntry(db, body.entry)
      } else if (body.replacement) {
        // ---- Замена участника: новому, снятому и напарнику ----
        return await notifyReplacement(db, body.replacement)
  ...
  // ============================================
  // Решение менеджера по заявке
  // ============================================
  //
  // Менеджер двигает заявку — в лист ожидания, обратно в сетку, отклоняет,
  ...
  async function notifyEntry(db: any, з: any): Promise<Response> {
    const { data: reg } = await db
      .from('tournament_registrations')
      .select('id, tournament_id, player_id, partner_id, partner_external_name, status')
      .eq('id', з.registration_id).single()
    if (!reg) return jsonResponse({ error: 'Registration not found' }, 404)

    const { data: tournament } = await db
      .from('tournaments').select('id, title').eq('id', reg.tournament_id).single()

    const кому = [reg.player_id, reg.partner_id].filter(Boolean)
    if (!кому.length) return jsonResponse({ ok: true, sent: 0, push_sent: 0, mode: 'entry' })

    const { data: profiles } = await db
      .from('profiles').select('id, player_id, telegram_chat_id, notify_preferences').in('player_id', кому)

    const тексты: Record<string, { заголовок: string; строка: string; значок: string }> = {
      waitlist: {
        значок: '\u{23F3}',
        заголовок: 'Вы в листе ожидания',
        строка: 'Заявка переведена в лист ожидания. Освободится место — вы попадёте в сетку по очереди'
      },
      draw: {
        значок: '\u{2705}',
        заголовок: 'Вы в основной сетке',
        строка: 'Место освободилось, и ваша заявка поднялась из листа ожидания'
      },
      rejected: {
        значок: '\u{26D4}',
        заголовок: 'Заявка отклонена',
        строка: 'Менеджер отклонил заявку. Вопросы — в клуб'
      },
      withdrawn: {
        значок: '\u{26A0}\u{FE0F}',
        заголовок: 'Заявка снята',
        строка: 'Менеджер снял вашу заявку с турнира. Вопросы — в клуб'
      },
      guest_ok: {
        значок: '\u{2705}',
      ...
      guest_removed: {
        значок: '\u{26A0}\u{FE0F}',
        заголовок: 'Напарника убрали из заявки',
        строка: 'Менеджер не подтвердил гостя. Место за вами — найдите другого напарника'
      }
    }

    const т = тексты[з.event]
    if (!т) return jsonResponse({ error: 'unknown_event' }, 400)
    ...
    for (const pr of (profiles || [])) {
      if (pr.telegram_chat_id && shouldNotify(pr.notify_preferences, 'tg', 'matches')) {
        await sendTg(pr.telegram_chat_id,
          `${т.значок} <b>${т.заголовок}</b>\n\n\u{1F3C6} ${esc(tournament?.title || '')}\n${esc(т.строка)}`)
        sent++
      }
      if (pr.id) {
        const ok = await sendPush(serviceKey, pr.id, т.заголовок, т.строка, reg.tournament_id)
        if (ok) pushSent++
      }
    }

    return jsonResponse({ ok: true, sent, push_sent: pushSent, mode: 'entry' })
  }
  ...
  // ============================================
  //
  // Менеджер меняет игрока на том же месте в сетке. Молча этого делать нельзя:
  // новый не знает, что заявлен, снятый может приехать зря, а напарник — что
  // теперь играет с другим человеком.
  async function notifyReplacement(db: any, з: any): Promise<Response> {
    const { data: tournament } = await db
      .from('tournaments').select('id, title, date_start').eq('id', з.tournament_id).single()
    if (!tournament) return jsonResponse({ error: 'Tournament not found' }, 404)

    const { data: reg } = await db
      .from('tournament_registrations')
      .select('id, player_id, partner_id, partner_external_name')
      .eq('id', з.registration_id).single()

    const кому: string[] = []
    if (з.new_player_id) кому.push(з.new_player_id)
    if (з.old_player_id) кому.push(з.old_player_id)
    if (reg?.player_id) кому.push(reg.player_id)
    if (reg?.partner_id) кому.push(reg.partner_id)
    const список = [...new Set(кому)]
    if (!список.length) return jsonResponse({ ok: true, sent: 0, push_sent: 0, mode: 'replacement' })

    const { data: players } = await db.from('players').select('id, name').in('id', список)
    const { data: profiles } = await db
      .from('profiles').select('id, player_id, telegram_chat_id, notify_preferences').in('player_id', список)

    const имя: Record<string, string> = {}
    for (const p of (players || [])) имя[p.id] = p.name
    const учётка: Record<string, any> = {}
    for (const p of (profiles || [])) учётка[p.player_id] = p

    const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    const парой = (id: string | null | undefined) => {
      if (!reg) return ''
      const первый = имя[reg.player_id] || ''
      const второй = reg.partner_id ? (имя[reg.partner_id] || '') : (reg.partner_external_name || '')
      return второй ? `${первый} / ${второй}` : первый
    }

    let sent = 0, pushSent = 0
    const отправить = async (кому: string, заголовок: string, текст: string) => {
      const pr = учётка[кому]
      if (!pr) return
      if (pr.telegram_chat_id && shouldNotify(pr.notify_preferences, 'tg', 'matches')) {
        await sendTg(pr.telegram_chat_id,
          `\u{1F504} <b>${заголовок}</b>\n\n\u{1F3C6} ${esc(tournament.title)}\n${esc(текст)}`)
        sent++
      }
      if (pr.id) {
        const ok = await sendPush(serviceKey, pr.id, заголовок, текст, tournament.id)
        if (ok) pushSent++
      }
    }

    const состав = парой(з.registration_id)

    if (з.new_player_id) {
      await отправить(з.new_player_id, 'Вы заявлены в турнир',
        состав ? `Ваша пара: ${состав}` : 'Вы включены в состав участников')
    }
    if (з.old_player_id) {
      await отправить(з.old_player_id, 'Вы сняты с турнира',
        'Вас заменили в составе участников. Вопросы — к менеджеру клуба')
    }
    // Напарнику, если меняли не его
    for (const свой of [reg?.player_id, reg?.partner_id]) {
      if (!свой || свой === з.new_player_id || свой === з.old_player_id) continue
      const новичок = з.new_player_id ? (имя[з.new_player_id] || '') : (з.new_name || '')
      await отправить(свой, 'В вашей паре замена',
        новичок ? `Теперь с вами играет ${новичок}` : 'Состав вашей пары изменился')
    }

    return jsonResponse({ ok: true, sent, push_sent: pushSent, mode: 'replacement' })
  }
