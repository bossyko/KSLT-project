        ...
        async function handleCallbackQuery(query: { id: string; data?: string; from: { id: number }; message?: { chat: { id: number }; message_id: number } }) {
          ...
          const newsMatch = data.match(/^news_(pub|skip|undo):([0-9a-f-]+)$/)
          if (newsMatch) {
        ...
        async function handleNewsDecision(
          ...
          // Кто-то из менеджеров уже ответил. Возврата это не касается:
          // его как раз нажимают на пропущенной находке.
          if (действие !== 'undo' && (s.status === 'published' || s.status === 'skipped')) {
          ...
          // Текст сообщения уже может нести прежнюю отметку — срезаем её,
          // иначе после возврата их накопится несколько подряд
          const исходный = (query.message?.text || '')
            .replace(/\n+(✖️ Пропущено|✅ Опубликовано на сайте|↩️ Возвращено в очередь).*$/s, '')
          ...
          if (действие === 'skip') {
            ...
            if (chatId && messageId) {
              // Кнопки рядом, промахнуться легко — оставляем путь назад
              await пометить(token, chatId, messageId, экранировать(исходный),
                '<b>✖️ Пропущено</b>',
                { inline_keyboard: [[{ text: '↩️ Вернуть', callback_data: `news_undo:${id}` }]] })
            }
            await предложитьСледующую()
            return
          }

          // ---- Передумал ----
          // Возвращаем находку в очередь: счётчик показов не трогаем, иначе
          // случайный промах съедал бы одну из трёх попыток.
          if (действие === 'undo') {
            await supabase.from('news_suggestions').update({
              status: 'new',
              decided_by: null,
            ...
            await answerCallbackQuery(token, query.id, 'Вернули')
            // Возвращаем сообщение ровно к прежнему виду, с обеими кнопками:
            // «вернули в очередь» ничего не говорит человеку — непонятно,
            // когда эта статья придёт снова и придёт ли вообще. Решение
            // должно приниматься здесь же.
            if (chatId && messageId) {
              await пометить(token, chatId, messageId, экранировать(исходный), '', {
                inline_keyboard: [[
                  { text: '✅ Опубликовать', callback_data: `news_pub:${id}` },
                  { text: '✖️ Пропустить',  callback_data: `news_skip:${id}` }
                ]]
              })
            }
            return
          ...
          const { error } = await supabase.from('news').insert({
            ...
            category: 'world',
            author: 'КСЛТ',
            // Ссылка отдельным полем, а не только внутри текста: по ней карточка
            // уводит читателя прямо к статье. Своей страницы у мировой новости
            // нет — наполнить её нечем, чужой текст мы не перепечатываем
            source_url: s.link,
        ...
        async function handleTournamentRegister(
          ...
          // 2. Check player_id
          //
          // Карточка заводится при входе на сайте или в приложении. Если её
          // нет, человек ничего не может: раньше он видел глухое «нет карточки»
          // и не понимал, куда идти. Говорим прямо
          if (!profile.player_id) {
            await answerCallbackQuery(token, query.id,
              'Откройте кабинет на сайте — карточка игрока заведётся сама, после этого запись заработает')
            return
          }
        ...
        /**
         * Дописать к сообщению отметку о решении.
         *
         * Всплывающего ответа мало: он гаснет через секунду, а в переписке
         * остаётся то же предложение без следа. Через день непонятно, публиковал
         * ты эту новость или прошёл мимо. Поэтому правим сам текст.
         */
        async function пометить(token: string, chatId: number, messageId: number,
                                текст: string, отметка: string, кнопки?: unknown) {
          await fetch(`${TELEGRAM_API}${token}/editMessageText`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              reply_markup: кнопки || { inline_keyboard: [] }
            })
          })
        }
    
        /**
         * Предложить следующую находку сразу.
         *
         * Пропустил — значит человек в переписке и готов решать дальше. Ждать
         * до следующего срока незачем: за один заход можно перебрать несколько
         * и опубликовать подходящую.
         *
         * Зовём ту же функцию, что и расписание: правило выбора должно жить
         * в одном месте.
         */
        async function предложитьСледующую() {
          try {
            await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/news-offer`, {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')}`,
                'Content-Type': 'application/json'
            ...
            // Не беда: следующая придёт по расписанию
            console.error('[news] следующую предложить не вышло:', e)
          }
        }
