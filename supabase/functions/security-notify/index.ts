...
await загрузитьТексты(supabase)
    ...
    const { data: profile } = await serviceClient
      .from('profiles')
      .select('full_name, email, telegram_chat_id, notify_preferences, lang')
      .eq('id', user.id)
      .single()
    ...
    if (profile.email && shouldNotify(profile.notify_preferences, 'email', 'security')) {
      ...
      emailSent = await callSendEmail(serviceKey, {
        to: profile.email,
        subject: т('mail_security_subject', языкИз(profile)),
        template: 'security-alert',
        data: { name, event: eventType, message_ru: ruMsg, message_en: enMsg, lang: языкИз(profile) }
      })
    }
