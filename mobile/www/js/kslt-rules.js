...
(function () {
    ...
    /**
     * Место игрока в категории — так же, как его считает рейтинг.
     *
     * Категории в базе общие для обоих полов: `masters` — и мужской, и
     * женский. Рейтинг разводит их вкладками «Мужчины / Женщины», а
     * страница игрока и кабинет считали подряд по всей категории — и
     * женщина с 660 очками получала 13-е место вместо 5-го, потому что
     * впереди оказывались восемь мужчин.
     *
     * Один расчёт на все три места: рейтинг, страницу игрока и кабинет.
     * Без пола (у карточки он не заполнен) считаем как раньше — по всей
     * категории: это лучше, чем не показать место вовсе.
     */
    R.местоВКатегории = async function(client, категория, очки, пол) {
        if (!client || !категория) return null;

        var запрос = client.from('player_categories')
            .select('player_id' + (пол ? ', players!inner(gender)' : ''), { count: 'exact', head: true })
            .eq('category_id', категория)
            .gt('points', очки || 0);

        if (пол) запрос = запрос.eq('players.gender', пол);

        var ответ = await запрос;
        return (ответ.count || 0) + 1;
    };
