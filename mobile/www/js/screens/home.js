...
(function() {
  ...
  function openTournamentDetail(tid) {
    ...
    supabaseClient.from('tournaments').select('*').eq('id', tid).single().then(function(r) {
      ...
      if (t.location) info += tdInfoRow('📍', I18N.t('td.location'), t.location);
      if (t.max_participants) info += tdInfoRow('👥', I18N.t('td.participants'), t.max_participants + ' ' + I18N.t('td.max'));
    ...
    supabaseClient.from('tournament_registrations')
      ...
      .then(function(r) {
        ...
        regs.forEach(function(reg, i) {
          var pName = reg.player ? reg.player.name : I18N.t('home.player');
  ...
  function loadParticipants(tournamentId) {
    ...
    supabaseClient.from('tournament_registrations')
      ...
      .then(function(r) {
        ...
        var html = '<div style="margin-top:16px"><div class="pd-section-title" style="font-size:14px;font-weight:600;margin-bottom:8px">' + I18N.t('td.participants') + ' (' + r.data.length + ')</div>';
