...
/**
 * Граница коротких подписей — своя, 600, а не 768.
 *
 * Сокращать пришлось потому, что полные названия занимают 457, а на 390
 * доступно 358. На планшете доступно 736, и полные влезают с запасом 280 —
 * сокращать там нечего. Расчёт: 600 даёт запас 111, 500 — всего 11, 480 не
 * влезает вовсе.
 */
function узкий(page) {
...
test.describe('Подвал — разметка и содержимое · ' + имя, () => {
    ...
    /* ПЕРЕПИСАНО 24.09 по решению Кости: строка подвала — просто «Сделано в
       Кыргызстане», без значков. Раньше тест требовал, чтобы значки были
       скрыты от диктора; теперь требует, чтобы их не было. */
    test('в строке подвала нет значков вовсе', async ({ page }) => {
        await кПодвалу(page, адрес);
        const строка = page.locator('.footer-made');
        await expect(строка).toHaveCount(1);
        const итог = await строка.evaluate(el => ({
            детей: el.children.length,
            эмодзи: /[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE0F}]/u.test(el.textContent),
            текст: el.textContent.trim()
        }));
        expect(итог.эмодзи, 'в строке «' + итог.текст + '» остался значок').toBe(false);
        expect(итог.детей, 'в строке «' + итог.текст + '» остались вложенные элементы').toBe(0);
    });
    ...
    test('полные названия там, где влезают; короткие — на узком', async ({ page }) => {
        ...
        if (узкий(page)) {
            expect(видно.короткое, 'на узком экране должна быть короткая подпись').toBe(true);
            expect(видно.полное, 'полная подпись на узком экране ломает ряд').toBe(false);
        } else {
            expect(видно.полное, 'место есть — название должно быть полным').toBe(true);
