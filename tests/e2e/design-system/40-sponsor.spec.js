...
СТРАНИЦЫ.forEach(({ имя, адрес }) => {
    test.describe('«Станьте спонсором» · ' + имя, () => {
        ...
        test('четыре числа, четыре обещания, один призыв', async ({ page }) => {
            ...
            /* ДОЛГ ЗАКРЫТ 24.09: страница спонсоров переведена на английский и
               кыргызский, и призыв ведёт на страницу своего языка. */
            const суф = имя === 'ru' ? '' : '-' + имя;
            expect(с.кнопка.адрес, 'адрес призыва · ' + имя).toBe('pages/sponsors' + суф + '.html');
        });
