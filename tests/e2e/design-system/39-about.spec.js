...
СТРАНИЦЫ.forEach(({ имя, адрес, суф }) => {
    test.describe('«О проекте» · ' + имя, () => {
        ...
        /* ПОПРАВКА 24.09, вечер: здесь стояло «сетка во всю ширину тела».
           Решение Кости это отменило — плитки остаются в левой колонке, а
           снимок встаёт справа РОСТОМ С НЕЁ. Проверяем новое отношение. */
        test('на широком снимок ростом с левую часть', async ({ page }) => {
            await дождаться(page, адрес);
            const с = await снимок(page);
            expect(с, 'раздел #about на странице').not.toBeNull();
            if (с.экран.ш > 992) {
                const левая = с.сетка.н - с.лид.в;
                expect(Math.abs(с.картинка.вы - левая),
                    'снимок ' + с.картинка.вы + ', левая часть ' + левая).toBeLessThanOrEqual(3);
                expect(с.сетка.ш, 'сетка уже тела: ' + с.сетка.ш + ' из ' + с.тело.ш)
                    .toBeLessThan(с.тело.ш);
            } else {
                expect(с.сетка.ш, 'на узком сетка во всю ширину тела').toBe(с.тело.ш);
            }
        });
        ...
        test('порядок по смыслу: вступление, плитки, снимок', async ({ page }) => {
            ...
            if (с.экран.ш <= 992) {
                ...
                expect(с.картинка.в, 'на широком снимок начинается на одной высоте с лидом')
                    .toBeLessThanOrEqual(с.лид.в + 2);
            }
        });
